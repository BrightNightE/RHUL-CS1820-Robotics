...
public class Driver {
	...
	final static float[] LEFT_SAMPLES = new float[1];

	final static SampleProvider RIGHT_SP = UR.getDistanceMode();
	final static float[] RIGHT_SAMPLES = new float[1];

	final static SampleProvider COLOUR_SP = CS.getColorIDMode();
	final static float[] COLOUR_SAMPLES = new float[1];
	...
	public static void startup() {
		...
		final Wheel WHEEL_RIGHT = WheeledChassis.modelWheel(MR, MoveList.WHEEL_DIAMETER.getMove())
				.offset(MoveList.AXEL_LENGTH.getMove() / 2).invert(true);
		final Chassis chassis = new WheeledChassis((new Wheel[] { WHEEL_RIGHT, WHEEL_LEFT }), WheeledChassis.TYPE_DIFFERENTIAL);
